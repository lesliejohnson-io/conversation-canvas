...
from ..services.otter_ingest import parse_otter_txt_to_speaker2_sentences
...
async def ingest_otter(file: UploadFile = File(...)) -> dict:
    ...
    ingest = parse_otter_txt_to_speaker2_sentences(txt)
    ...
    with DBSession(engine) as db:
        ...
        for idx, sentence_text in enumerate(ingest.sentences):
            db.add(
                Sentence(
